...
def build_and_run_simulations(samples, scenario_template_sets, scenario_param_dicts, suite_name, loaded_module):
    ...
    for template_set_name, scenario_template_set in scenario_template_sets.items():
        ...
        for scenario_params in scenario_param_dicts:
            ...
            while(too_many_jobs):
                time.sleep(60)
                if len(experiment_managers) < 7:
                    too_many_jobs = False
                for i_experiment_manager in range(len(experiment_managers)-1, max(len(experiment_managers) - 7, 0), -1) :
                    try:
                        status = slurm.status_for_job_id(experiment_managers[i_experiment_manager].job_id)
                    except:
                        status = slurm.EntityStatus.SUCCEEDED
                    if status  == slurm.EntityStatus.SUCCEEDED :
                        too_many_jobs = False 
                print(f"Too Many Jobs, waiting 60 seconds before starting the next")
...
def parse_args():
    ...
    parser.add_argument('-d', '--calib-dir', dest='calibration_dir', type=str, required=False,
                        help='Directory of calibration to resample (Required).')
